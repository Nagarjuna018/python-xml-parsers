    ...
    for filename in tqdm(os.listdir(directory)):
        if filename.endswith('.xml'):
            ...
            for datapoint_bbox in datapoint_bboxes:
                for box in boxes:
                    if percent_inside(box, datapoint_bbox) >= 0.95:
                        is_inside_table = any(percent_inside(box, table_bbox) >= 0.95 for table_bbox in table_bboxes)
                        is_inside_header = any(percent_inside(box, header_bbox) >= 0.95 for header_bbox in header_bboxes)
                        if not is_inside_table and not is_inside_header:
                            box['element'].find('name').text = 'combo_block'
            
            tree.write(os.path.join(directory, filename))

if __name__ == '__main__':
    directory = "/home/nagarjuna/Desktop/PO_PROJECT_2/images_segregation/rename_need_with_script"  
